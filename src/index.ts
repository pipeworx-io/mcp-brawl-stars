...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Brawl Stars');
}
...
const tools: McpToolExport['tools'] = [
  { name: 'player', description: 'Brawl Stars official API — full profile for a player tag (#9LJ2YR9…): trophies, highest trophies, club, brawler count, 3v3 wins, solo/duo wins, and experience level.', inputSchema: { type: 'object', properties: { tag: { type: 'string' } }, required: ['tag'] } },
  { name: 'player_battles', description: 'Brawl Stars official API — recent battle log for a player tag: battle time, event/map, mode, result, star player, teams, and brawlers used per battle.', inputSchema: { type: 'object', properties: { tag: { type: 'string' } }, required: ['tag'] } },
  { name: 'club', description: 'Brawl Stars official API — full club profile for a club tag: name, description, type, trophies required, total trophies, and member count.', inputSchema: { type: 'object', properties: { tag: { type: 'string' } }, required: ['tag'] } },
  {
    name: 'club_members',
    description: 'Brawl Stars official API — paginated member list for a club tag: each member\'s name, tag, trophies, role, and name color. Supports limit/after/before cursors.',
    inputSchema: { type: 'object', properties: { tag: { type: 'string' }, limit: { type: 'number' }, after: { type: 'string' }, before: { type: 'string' } }, required: ['tag'] },
  },
  { name: 'brawlers', description: 'Brawl Stars official API — complete list of all brawlers with their numeric ID, name, star powers, and gadgets.', inputSchema: { type: 'object', properties: {} } },
  { name: 'brawler', description: 'Brawl Stars official API — detail for a single brawler by numeric ID: name, star powers with descriptions, and gadgets with descriptions.', inputSchema: { type: 'object', properties: { id: { type: 'number' } }, required: ['id'] } },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!apiKey) throw new Error('Brawl Stars requires an API key: pass your key as the _apiKey argument (free at https://developer.brawlstars.com — generate with "Allow any IP").');
  const get = async (path: string, params?: Record<string, unknown>) => {
    ...
    const res = await pwFetch(url, { headers: { Accept: 'application/json', 'User-Agent': UA, Authorization: `Bearer ${apiKey}` } });
    ...
    if (!res.ok) throw await httpError(res, 'Brawl Stars');
    return res.json();
  };
